...
/**
 * Tulis ulang tab "Dashboard" agar total aset per rekening dan grand total
 * selalu terlihat di sheet itu sendiri (bukan cuma di balasan WhatsApp).
 * Sheet "Dashboard" perlu ada duluan (tab kosong juga tidak masalah, akan
 * diisi otomatis oleh fungsi ini).
 */
async function writeDashboard(accounts) {
  const sheets = getSheetsClient();
  const sheetId = process.env.GOOGLE_SHEET_ID;
  const dash = computeDashboard(accounts);

  const header = ["ID", "Bank", "Pemilik", "Kategori", "Saldo Riil / Kontribusi Aset"];

  const rows = accounts.map((a) => [a.id, a.bank, a.pemilik, a.kategori, a.saldoRiil]);

  const summary = [
    ["", "", "", "", ""],
    ["", "", "", "Total Saldo Rekening Sendiri", dash.totalSendiri],
    ["", "", "", "Total Piutang Aktif", dash.totalPiutang],
    ["", "", "", "Total Aset Asli", dash.totalAsetAsli],
  ];

  const values = [header, ...rows, ...summary];

  // Bersihkan dulu supaya baris lama (kalau jumlah rekening berkurang) tidak nyangkut
  await sheets.spreadsheets.values.clear({
    spreadsheetId: sheetId,
    range: "Dashboard!A1:E200",
  });
...
module.exports = { loadAccounts, resolveAccount, updateBalance, computeDashboard, writeDashboard };
